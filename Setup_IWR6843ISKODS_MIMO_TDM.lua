NUM_TX = 3
NUM_RX = 4
mmwave_root = "C:\\ti\\mmwave_studio_02_01_01_00\\"


COM_PORT = 6
START_FREQ = 60  -- GHz
ADC_START_TIME = 6  -- micro sec
FREQ_SLOPE = 80  -- MHz/microsec
ADC_SAMPLES = 256
SAMPLE_RATE = 6000  -- ksps
IDLE_TIME = 50  -- micro sec
RAMP_END_TIME = 50 -- micro sec

CHIRP_LOOPS = 128
PERIODICITY = 39 -- milli sec, duration of a frame
NumOfFrame = 513  -- 513 = 20 seconds

-- autoStart = 1
NumOfProfile = 1  -- there is one profile for all chirps
NumOfChirpInLoop = 3  -- 3 Tx, each Tx has one chirp

TxSeq = {}
TxSeq[1]={1,0,0}  -- Tx1 first
TxSeq[2]={0,1,0}  -- then Tx2
TxSeq[3]={0,0,1}  -- finally Tx3
profile={}
...
-- use below if different chirps are needed
...
fw_path   = mmwave_root.."rf_eval_firmware"
...
bitopfile = mmwave_root.."mmWaveStudio\\Scripts\\bitoperations.lua"
dofile(bitopfile)
...
data_path     = mmwave_root.."mmWaveStudio\\PostProc"
...
for i = 0, NumOfChirpInLoop - 1 do
    ...
    tx2 = TxSeq[i+1][2]
    tx3 = TxSeq[i+1][3]
    if (ar1.ChirpConfig(i, i, profileID, 0, 0, 0, 0, tx1, tx2, tx3) == 0) then
